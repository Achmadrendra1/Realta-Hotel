...
import { Affix, Breadcrumb, Button, Card, Carousel, InputNumber, Modal, Pagination, Radio, Select } from 'antd'
...
export default function menu({ restaurant }) {
    ...
    let numberOrder = useSelector( (state:any) => state.numberOrderReducer.numberOrder );
    ...
    const removeFromCart = (productToRemove:any) => {
        setCart(
            cart.filter((product:any) => product !== productToRemove)
        )
    }
  ...
  const setQuantity = (product:any, amount:any) => {
    ...
    const newCart = [...cart];
    newCart.find(
        (item) => item.remename === product.remename
    ).quantity = amount
    setCart(newCart)
  }
  ...
  const getTotalSum = ():number => {
    ...
    cart.map( (item:any) => {
        ...
        let numberOfPrice = Number(item.remeprice.replace(/[^0-9.-]+/g,""));
...
//   --------------------------------------------------------------------- MODALS
  ...
  async function showModalsMenu(menu:any){
    ...
    const result = await axios((API('Get', `/resto-menu-photos/${menu.remeid}`, null)))
...
//   console.log(menus);
  ...
  // ------------------------ SORTED
// console.log(menus);
  let sorted = menus.sort((a:any, b:any) => a.remeprice > b.remeprice ? 1 : -1);
  const [sortedData, setSorted] = useState('')

  if(sortedData && sortedData=='DESC'){
    sorted = menus.sort((a:any, b:any) => b.remeprice >  a.remeprice ? 1 : -1)
  }
        ...
        <Layouts>
            <div className='container mx-auto'>
                ...
                <div className='mt-5 mr-5 mb-5 flex'>
                    <Image src='/assets/resto/1.jpg' alt='resto' width={300} height={300}></Image>
                    <div className='m-5'>
                        <p className='text-2xl font-bold'>
                            {restaurant.hotel_name}
                        </p>
                        <p className='text-xl font-bold'>
                            {restaurant.faci_name}
                        </p>
                        <p>
                            {/* {faci_description} */}
                        </p>
                ...
                <div className='mt-3 flex my-20'>
                    <div className='w-3/5 border rounded shadow p-3 mr-2'>
                        ...
                        <div className='text-xl font-bold text-center'>Menu Makanan dan Minuman</div>
                        ...
                        <div className='flex flex-wrap ml-5 item-center'>
                      {
                        menusWPagination.filter((menu:any)=>{
                            ...
                            map( (menu:any, key:any) => 
                              ...
                              <div key={menu.remeid} className='w-52 mr-6 mx-auto mb-12'>
                                <div>
                                    <a onClick={() => showModalsMenu(menu)} className=' hover:text-slate-400'>
                                        <img src={`${configuration.BASE_URL}/${menu.rempurl}`} alt={menu.remename} className='h-40 w-full object-cover rounded-lg'>
                                        </img>
                                    ...
                                    {/* <div className='absolute'> */}
                                      ...
                                      { userid ? 
                                        <button onClick={()=>addtocart(menu)} className='w-3/4 mx-auto rounded-full inline-block px-5 py-2 mt-4 bg-slate-500 hover:bg-slate-600 text-white uppercase bottom-0'>
                        ...
                        <Pagination onChange={handlePagination} current={currentpage} pageSize={10} total={menus.length} className='text-center py-14' />
                ...
                //    <Affix className='w-2/5'>
                    <div className='border rounded shadow p-3 ml-2 sticky top-0 h-1/2 w-2/5'>
                        ...
                        <div className='border rounded py-5 bg-slate-100'>
                            <p className='font-bold text-center py-2'>Payment Summary</p>

                            <table className='py-5 bg-slate-100 mx-4'>
                                <tbody>
                                    <tr className='hover:bg-slate-300'>
                                        <td className='w-full'>Sub total</td>
                                        <td className='text-right'>Rp.{getTotalSum().toLocaleString('id-ID')}</td>
                                    </tr>
                                    {/* <tr className='hover:bg-slate-300'>
                                    ...
                                    <tr className='hover:bg-slate-300'>
                                        ...
                                        <td className='text-right'>{tax().toLocaleString('id-ID')}</td>
                                    </tr>
                                    <tr className='font-bold hover:bg-slate-300'>
                                        <td className=' py-2'>Total payment</td>
                                        <td className='text-right'>Rp.{sumWithTax().toLocaleString('id-ID')}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
                        {/* <Link href='order'> */}
                          <a>
                            <div onClick={placeOrder} className='border rounded-lg bg-slate-100 my-5 text-center font-bold py-2 hover:bg-slate-300 hover:text-white' >
                                PLACE ORDER HERE
                            </div>
                          </a>
                        {/* </Link> */}
                    ...
                    menuDetailPhoto.map( (photo:any) => (
                        ...
                        <div className='mr-3 transition ease-in-out delay-150 hover:scale-110 duration-300'>
                            <a onClick={()=>tampilkanPhoto(photo)}>
                                <div  className=' p-2'>
                                    <img src={photo.rempurl} alt={photo.rempthumbnailfilename} className='h-24 w-24 object-cover' /> 
                                </div>
                            </a>
                        </div>
                        </>
