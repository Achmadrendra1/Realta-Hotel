...
function payBankReducer(state = initialState, action: any) {
  switch (action.type) {
    ...
    case PaymentConst.GET_BANK_REQUEST_SUCCESS:
      return {...state, payBank: action.payload}
    ...
    case PaymentConst.UPDATE_BANK_SUCCESS:
      state.payBank.splice(state.payBank.findIndex((i:any) => i.bankEntityId == action.payload.bankEntityId), 1, action.payload)
      return {
        ...state,
    ...
    case PaymentConst.DELETE_BANK_SUCCESS:
      return {
        ...state,
        payBank : state.payBank.filter((items:any) => items.bankEntityId !== +action.payload)
      }
    default:
      return state
  }
