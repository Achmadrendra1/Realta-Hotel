...
select * from resto.listMenuUser(2, 'vege', 1, 'DESC') 

-- paging searching by postgre
...
select * from resto.ordermenuscomplete('MENUS#20230307-0001')
	...
	OPEN dataordermenus;
		LOOP FETCH NEXT FROM dataordermenus 
			INTO orme_id,orme_order_number,orme_order_date,orme_total_item,orme_total_discount,
				...
				omde_id,orme_price,orme_qty,orme_subtotal,omde_orme_id,omde_reme_id,reme_name;
		EXIT WHEN NOT FOUND;
		RETURN NEXT;
		END LOOP;
	CLOSE dataordermenus;
...
drop function resto.ordermenuscomplete
CREATE OR REPLACE FUNCTION resto.ordermenuscomplete(orderNumber text) 
	RETURNS TABLE (
		orme_id int,
		orme_order_number varchar(20),
...
SELECT setval('resto."order_menu_detail_omde_id_seq"', 
		(SELECT MAX(omde_id) FROM resto.order_menu_detail));

	
SELECT setval('resto."order_menus_orme_id_seq"', 
		(SELECT MAX(orme_id) FROM resto.order_menus));
		
	
SELECT setval('resto."resto_menu_photos_remp_id_seq"', 
		(SELECT MAX(remp_id) FROM resto.resto_menu_photos));
		
	
SELECT setval('resto."resto_menus_reme_id_seq"', 
		(SELECT MAX(reme_id) FROM resto.resto_menus));
...
CREATE OR REPLACE FUNCTION resto.getPhotoMenu(remeId int)
	RETURNS TABLE (
		rempId int, rempThumbnailFilename text, rempPhotoFilename text, rempPrimary bit, rempUrl text, rempReme int, remeName text
	)
	AS
	$$
	
	DECLARE
		dataPhoto CURSOR FOR(
			SELECT resto.resto_menu_photos.remp_id, resto.resto_menus.reme_id, resto.resto_menus.reme_name 
			FROM resto.resto_menu_photos
			JOIN resto.resto_menus ON resto.resto_menus.reme_id = resto.resto_menu_photos.remp_reme_id
			WHERE resto.resto_menus.reme_id = remeId
		);
	BEGIN
		OPEN dataPhoto;
			LOOP
				FETCH NEXT FROM dataPhoto
					INTO rempId, rempThumbnailFilename, rempPhotoFilename, rempPrimary, rempUrl, rempReme, remeName;
					EXIT WHEN NOT FOUND;
				RETURN NEXT;
			END LOOP;
		CLOSE dataPhoto;
...
-- 3. function resto.ordermenuscomplete(noorder,userid), ambil codenumber dan id user saat user order data
	...
	CREATE OR REPLACE FUNCTION resto.ordermenuscomplete(orderNumber text, user_id int)
	RETURNS TABLE (
		orme_id int, orme_order_number varchar(20), orme_order_date date, orme_total_item INT, orme_total_discount MONEY, orme_total_amount MONEY,
		orme_pay_type varchar(2), orme_cardnumber text, orme_is_paid varchar(2), orme_modified_date date, orme_user_id int,
		omde_id int, orme_price money, orme_qty int, orme_subtotal money, omde_orme_id int, omde_reme_id int,
		reme_name text
	)
	AS 
	$$
	...
	BEGIN
		OPEN dataordermenus;
			LOOP
				FETCH NEXT FROM dataordermenus
					INTO 	orme_id, orme_order_number, orme_order_date, orme_total_item, orme_total_discount, orme_total_amount,
							orme_pay_type, orme_cardnumber, orme_is_paid, orme_modified_date, orme_user_id,
							omde_id, orme_price, orme_qty, orme_subtotal, omde_orme_id, omde_reme_id, reme_name;
					EXIT WHEN NOT FOUND;
...
-- ORDERS
	CREATE OR REPLACE FUNCTION resto.orders(
			ormeOrderNumber text, ormeDate date, 
			ormeTotalItem int, ormeTotalDiscount int, ormeTotalAmount int, orme_pay_type varchar(2),
			ormeIsPaid text, ormeUserId int)
		RETURNS int
		AS
		$$
		...
		BEGIN
			INSERT INTO resto.order_menus(orme_order_number, orme_order_date, orme_total_item, orme_total_discount,
										  orme_total_amount, orme_pay_type, orme_is_paid, orme_modified_date, orme_user_id) 
			VALUES
				(ormeOrderNumber, ormeDate, ormeTotalItem, ormeTotalDiscount, ormeTotalAmount, orme_pay_type, ormeIsPaid, ormeDate, ormeUserId)
			RETURNING resto.order_menus.orme_id INTO id_orme;
			RETURN id_orme;
...
-- buat di dashboard
CREATE OR REPLACE VIEW resto.restomenu_dashboard AS
	SELECT DISTINCT ON (resto.resto_menus.reme_id) hotel.hotels.hotel_name, hotel.facilities.faci_id, hotel.facilities.faci_name,
			resto.resto_menus.reme_id, resto.resto_menus.reme_name, resto.resto_menus.reme_description, resto.resto_menus.reme_price,
			resto.resto_menus.reme_status, resto.resto_menus.reme_modified_date, resto.resto_menus.reme_discount, 
...
-- BANYAK DATA (used)
-- drop function resto.count_menu
CREATE OR REPLACE FUNCTION resto.count_menu(faci_id int, search_menu varchar(50))
	...
	BEGIN
		select count(*) 
		from resto.resto_menus 
		where LOWER(resto.resto_menus.reme_name) LIKE CONCAT('%',search_menu,'%')
...
-- SELECT * from resto.count_menu(9,'')


-- list menu di dashboard
-- drop view resto.restomenu_dashboard

select * from resto.restomenu_dashboard('air',1)
...
select * from resto.count_menu_dashboard('')
